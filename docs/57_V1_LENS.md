...
## 57.1 The 12 v1 items → phases → GitHub issues

**Canonical trace.** Issue numbers reference the Phase 1–3 milestones (M1–M3). Items in phases whose milestone is not yet created are marked **—** (create them with that milestone, referencing the item number below). docs/49 §49.14 is the phase-plan view of this same mapping.

| # | Handoff item | Where it lives | GitHub issue(s) | Status |
|---|---|---|---|---|
| 1 | CEO Home Screen (one screen) | Phase 3 (Executive screen chat mode) + Phase 4 (Work Center) | #30 (Executive screen chat) · Phase 4 Work Center — | Planned |
| 2 | Executive Agent | Phase 3 (Executive Agent workflow) | #26 (context → plan → recommend) | Planned |
| 3 | Hiring Flow (max 5 agents, "Hire" never "Create") | Phase 2 (hiring lifecycle, business_cases, agent templates 17d) | #19 (lifecycle) · #23 (events + audit) · #18 (templates + 17d seed) | Planned |
| 4 | Org Explorer (tree: CEO → Executive → Depts → Agents) | Phase 2 (organization explorer UI) | #22 | Planned |
| 5 | Goals + Tasks (status: ready/running/blocked/done) | Phase 4 (goals, projects, tasks, kanban/list) | — (create with Phase 4 milestone) | Planned |
| 6 | One Approval Gate (server-side, one level) | Phase 5 (Approval Engine) — Phase 1–3 queue approvals structurally | #12 (pg-boss runtime + outbox, Phase 1) · Phase 5 Approval Engine — | Planned |
| 7 | Model Router with BYOK (encrypted, masked, never logged) | Phase 7 (Model Gateway) — **provider config + encrypted secrets already built (Phase 1, docs/23/37)** | #9 (provider config + encrypted secrets) ✅ · Phase 7 routing — | 🔶 Built (key mgmt), routing in 7 |
| 8 | Basic Company Memory (pgvector, permission-aware) | Phase 3 (Company Memory, configurable EMBED_DIM) | #27 | Planned |
| 9 | Audit Trail (append-only) | Phase 1 (audit hash chain) + Phase 5 (audit UI) | #7 (audit framework: hash chain) | ✅ Built (append-only) |
| 10 | Weekly Report (one page) | **Pulled forward to Phase 3** (see 57.3) | — (add under Phase 3; gate-checked by #32 DoD) | Planned (moved) |
| 11 | One Integration (GitHub) | Phase 8 (GitHub integration — the v1 engineering path) | — (create with Phase 8 milestone) | Planned |
| 12 | Constitution (minimal, 3–5 rules, code-enforced) | Phase 5 (Constitution editor + enforcement); seed 17b/17c ready | — (create with Phase 5 milestone; seeds 17b/17c ready) | Planned |

**Verdict:** all 12 are already inside docs/49's phases. Nothing in the handoff requires a new phase — the only changes are the two pull-forwards below and the two corrections in 57.4. Items 1 and 7 span two phases: the listed issue is the primary build, the phase after the · is the follow-on surface.
...
| **Weekly CEO Report** (retention loop) | Adopted. Minimal version (goals progress, done/blocked, spend placeholder) built from audit trail + task tables | **Pulled forward to Phase 3** · — (add under Phase 3; gate-checked by #32) |
...
| **"Why did you do that?" explanation layer** | Adopted. One-sentence "because" inline on recommendations/delegations/approval requests; Executive generates it | Phase 3 (explain-why) · **#28** (inline "because" checklist item) |
...
- **GitHub:** milestone checklists for Phases 2–5 should reference the item numbers above (e.g., "57.1-#4 Org Explorer"). **§57.1 above is the canonical phase → issue trace for the 12 items**; docs/49 §49.14 mirrors it as the phase-plan view. When creating a missing milestone, seed its issues from the **—** rows and reference the item number in each title.
