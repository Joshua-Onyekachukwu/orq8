...
### Session — Phase 10/11 completion: Business Import, build-vs-buy resolve, Executive Agent capability injection

- **Business Import (Phase 10)** — `supabase/migrations/0013_add_business_imports.sql` + schema mirror (`business_imports`: org-scoped, `(org_id, source_fingerprint)` unique for idempotent re-analysis, facts/proposal jsonb, status lifecycle). `services/business-import.ts`: SSRF-guarded website fetch (private-IP/metadata/loopback blocked incl. IPv4-mapped, per-hop redirect revalidation, bounded size/time/text), deterministic provenance-backed fact extraction (industry/model/audience/channels/geography/stage with confidence + snippet + source), org proposal (recommends real playbook by signal: ecommerce-growth / agency-operations / startup-launch), founder approval gate → apply via existing idempotent `seedPlaybook` (org builder) + Company Brain persistence (facts → company memory with provenance, import node → knowledge graph, audit). Routes `/v1/business-imports*` (analyze/propose/approve/reject/list/detail; approve+reject owner/admin-only). Web: proxies + `/app/business-import` page + sidebar entry.
- **Capability registry build-vs-buy resolution (Phase 11)** — `resolveCapabilityRequest` over registered capabilities + active AI employees + company knowledge (bounded, org-scoped, token-overlap confidence, reuse/extend/build decision, audit `capability.resolve` with truncated request). `POST /v1/capabilities/resolve` + web proxy. **Executive Agent integration**: `ExecutiveContext.capabilities` — resolves the founder query and injects matches + decision into the context prompt; otherwise lists reusable catalog; instructions now mandate search-before-build.
- **Verification** — API typecheck 0 errors, **402 tests passing** (business-import: SSRF/URL/extraction/fetch/proposal unit + DB-gated lifecycle & isolation; capability-resolve unit + DB-gated), web typecheck clean, production web build passing. DB-gated integration tests auto-skip until migration **0013 is applied** to the DB the tests run against.
### Session — Central Entitlement Engine + plan-aware workforce sizing

- **Single source of truth for plan limits**: canonical AI employee limits fixed at Founder 10 / Team 25 / Company 50 (decision: match live enforcement + stored rows; marketing copy in `landing/Common/Pricing.tsx`, `core/pricing.ts` and billing feature labels aligned to 10/25/50). `services/billing.ts PLANS` extended with structured resource caps (departments / teams / connectors / custom MCP servers / autonomy ceiling) + `TRIAL_CAPS`; removed the marketing-vs-enforcement numeric drift.
