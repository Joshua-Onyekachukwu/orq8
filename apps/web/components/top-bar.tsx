      ...
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-gray-100 bg-white/95 backdrop-blur-sm px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-[13px] text-gray-400 transition-colors hover:border-gray-300 hover:bg-gray-100"
          >
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              <div className="h-8 w-8 rounded-full bg-[#1a5c2e] flex items-center justify-center text-[12px] font-bold text-[#B8FF66]">
                {initials}
              </div>
            ...
            {profileOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-gray-100 bg-white py-2 shadow-lg">
                ...
                <div className="py-1">
                  <Link href="/app/profile" className="flex items-center gap-2 px-4 py-2 text-[13px] text-gray-600 hover:bg-gray-50" onClick={() => setProfileOpen(false)}>
                    <User className="h-4 w-4" /> Profile
                  </Link>
                  <Link href="/settings" className="flex items-center gap-2 px-4 py-2 text-[13px] text-gray-600 hover:bg-gray-50" onClick={() => setProfileOpen(false)}>
                    <Settings className="h-4 w-4" /> Settings
                  </Link>
                  {platformRole === "admin" && (
                    <Link href="/admin" className="flex items-center gap-2 px-4 py-2 text-[13px] text-[#E86A33] hover:bg-[#E86A33]/5" onClick={() => setProfileOpen(false)}>
                      <Command className="h-4 w-4" /> Admin Dashboard
                    </Link>
                  )}
                </div>
                <div className="border-t border-gray-100 pt-1">
                  <Link href="/api/auth/logout" className="flex items-center gap-2 px-4 py-2 text-[13px] text-gray-500 hover:bg-gray-50">
