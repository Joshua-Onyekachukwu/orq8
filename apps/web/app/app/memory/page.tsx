      ...
      {stats && (
        <div className="mt-6 grid grid-cols-4 gap-4">
          <div className="rounded-xl border border-hairline bg-white p-4 text-center">
            ...
            <p className="text-3xs font-medium uppercase tracking-wide text-muted">Total</p>
          </div>
          {Object.entries(stats.byCategory).slice(0, 3).map(([cat, count]) => (
            <div key={cat} className="rounded-xl border border-hairline bg-white p-4 text-center">
              ...
              <p className="text-3xs font-medium uppercase tracking-wide text-muted">{cat}</p>
            </div>
          ))}
        </div>
      )}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`rounded-full px-2 py-0.5 font-mono text-3xs font-semibold uppercase ${categoryColor(entry.category)}`}>
                      {entry.category}
                    </span>
                    ...
                    {entry.source && (
                      <span className="text-3xs text-muted">via {entry.source}</span>
                    )}
                  </div>
